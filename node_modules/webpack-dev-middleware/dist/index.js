...
function wdm(compiler, options = {}, isPlugin = false) {
  ...
  if (options.hot) {
    context.hot = createHot(compiler, options.hot === true ? {} : options.hot, options.stats);
  }
  ...
  instance.invalidate = (callback = noop) => {
    ...
    if (invalidating.length === 0) {
      filledContext.logger.warn("The `invalidate` method was called, but the compiler is not watching, so there is nothing to invalidate.");
    ...
    try {
      ...
      (resolve, reject) => {
        ...
        chunk => {
          ...
          try {
            sseController.enqueue(typeof chunk === "string" ? new TextEncoder().encode(chunk) : chunk);
          } catch {
            return false;
          }
          return true;
